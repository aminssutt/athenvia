...
export type UniversitySubmissionRateLimit = {
  allowed: boolean;
  limit: number;
  remaining: number;
...
export class InMemorySubmissionRateLimiter {
  ...
  take(clientKey: string, userKey: string, now = Date.now()): UniversitySubmissionRateLimit {
...
type RateLimitState = {
  memory: InMemorySubmissionRateLimiter;
  redis?: Redis;
  warnedAboutFallback?: boolean;
};

const globalForRateLimit = globalThis as typeof globalThis & {
  athenviaUniversitySubmissionRateLimit?: RateLimitState;
};

const state =
  globalForRateLimit.athenviaUniversitySubmissionRateLimit ??
  ({ memory: new InMemorySubmissionRateLimiter() } satisfies RateLimitState);

if (process.env.NODE_ENV !== "production") {
  globalForRateLimit.athenviaUniversitySubmissionRateLimit = state;
}
...
function opaqueKey(kind: "client" | "user", value: string) {
  const secret =
    process.env.UNIVERSITY_SUBMISSION_RATE_LIMIT_SALT ??
    process.env.AUTH_SECRET ??
    "athenvia-submission-development";
...
async function takeFromRedis(
  clientKey: string,
  userKey: string,
): Promise<UniversitySubmissionRateLimit | null> {
...
export async function checkUniversitySubmissionRateLimit(
  request: Request,
  userId: string,
): Promise<UniversitySubmissionRateLimit> {
...
export function universitySubmissionRateLimitHeaders(
  rateLimit: UniversitySubmissionRateLimit,
): HeadersInit {
  return {
